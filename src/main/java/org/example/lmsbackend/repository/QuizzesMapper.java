...
public interface QuizzesMapper {
    ...
    @Insert("INSERT INTO quizzes (course_id, title, description, time_limit, publish) " +
            "VALUES (#{courseId}, #{title}, #{description}, #{timeLimit}, #{publish})")
    ...
    @Update("UPDATE quizzes SET title = #{title}, description = #{description}, " +
            "time_limit = #{timeLimit}, publish = #{publish} WHERE quiz_id = #{quizId}")
